...
tarot_cards = [
    ...
    "Иерофант (Жрец)", "Влюблeнные", "Колесница", "Сила", "Отшельник",
    "Колесо Фортуны", "Справедливость", "Повешенный", "Смерть",
    "Умеренность", "Дьявол", "Башня", "Звезда", "Луна", "Солнце",
    "Суд", "Мир",
    
    # Младшие Арканы - Жезлы
    "Туз Жезлов", "Двойка Жезлов", "Тройка Жезлов", "Четвeрка Жезлов",
    "Пятeрка Жезлов", "Шестeрка Жезлов", "Семeрка Жезлов", "Восьмeрка Жезлов",
    "Девятка Жезлов", "Десятка Жезлов", "Паж Жезлов", "Рыцарь Жезлов",
    "Королева Жезлов", "Король Жезлов",
    
    # Младшие Арканы - Кубки
    "Туз Кубков", "Двойка Кубков", "Тройка Кубков", "Четвeрка Кубков",
    "Пятeрка Кубков", "Шестeрка Кубков", "Семeрка Кубков", "Восьмeрка Кубков",
    "Девятка Кубков", "Десятка Кубков", "Паж Кубков", "Рыцарь Кубков",
    "Королева Кубков", "Король Кубков",
    
    # Младшие Арканы - Мечи
    "Туз Мечей", "Двойка Мечей", "Тройка Мечей", "Четвeрка Мечей",
    "Пятeрка Мечей", "Шестeрка Мечей", "Семeрка Мечей", "Восьмeрка Мечей",
    "Девятка Мечей", "Десятка Мечей", "Паж Мечей", "Рыцарь Мечей",
    "Королева Мечей", "Король Мечей",
    
    # Младшие Арканы - Пентакли
    "Туз Пентаклей", "Двойка Пентаклей", "Тройка Пентаклей", "Четвeрка Пентаклей",
    "Пятeрка Пентаклей", "Шестeрка Пентаклей", "Семeрка Пентаклей", "Восьмeрка Пентаклей",
...
def date_parsing(raw_data: str) -> dict:
    """
...
def draw_random_cards(tarot_list: list[str] = tarot_cards, N: int = 3) -> list[str]:
    ...
    result_list = [
        card + " перевeрнута" if random.random() < 0.25 else card
...
def get_tarot(model, data: dict, N: int = 3):
    ...
    tarot = date_parsing(model.invoke(prompt).content)
    return tarot



def cleaner(letter: str) -> str:
    """
    Удаляет все специальные символы MD созданные моделью
    """ 
    cleaned_letter = re.sub(r"[#*`]", "", letter)
